    ...
    AddCarPath(_nodePos: cc.Vec2) {
        ...
        if (pathList.length > 0) {
            ...
            if (count) {
                ...
                // Utils.CCLog("floor _nodePos x", _nodePos.x, "_nodePos y ", _nodePos.y);
            ...
            //循环判断
            // for (let index = 1; index <= count; index++) {
            //     let nodepos = _dir.normalize().scale(cc.v2(this.m_MoveLength * index, this.m_MoveLength * index));
            //     Utils.CCLog("nodepos", nodepos);
            // }
