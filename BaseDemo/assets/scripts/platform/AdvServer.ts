...
export default class AdvServer {
    ...
    private m_AdvConfigVersion = "20200813104634256267";
    ...
    private m_WeChatBannerID: string = "";    //微信banner id
    private m_WeChatBannerIndex: number = 0;  //微信banner id 下标
    ...
    private m_TgTypeFullBanner: boolean = false;
    ...
    public InitAdvServer(_onSuccess: Function = null, _onFail: Function = null) {
        ...
        // }
        {
            ...
            this.LoadAdvConfig(() => {
                ...
                if (BasePlatform.getInstance().IsWeChat()) {
                    //第三方跳转
                    let launchOptions = WeChat.GetLaunchOptionsSync();
                    if (launchOptions && !Utils.IsNull(launchOptions.referrerInfo)) {
                        let appid = launchOptions.referrerInfo.appId;
                        if (!appid) {
                            this.AdvLog("第三方屏蔽");
                            //开始金蛋
                            this.m_GlobalConfig.activity_startluckyegg = 0;
                            //结束金蛋
                            this.m_GlobalConfig.activity_endluckyegg = 0;
                            //开始视频
                            this.m_GlobalConfig.activity_startvideo = 0;
                            //结束视频
                            this.m_GlobalConfig.activity_endvideo = 0;
                        }
                    }
                    else {
                        this.AdvLog("第三方屏蔽");
                    ...
                    //省份屏蔽
                    if (this.m_GlobalConfig['city-config']) {
                        this.AdvLog("省份屏蔽");
                        //开始金蛋
                        this.m_GlobalConfig.activity_startluckyegg = 0;
                        //结束金蛋
    ...
    public ShowMenuAdv(_menu: string, _parent: cc.Node = null) {
        ...
        if (_menu == BaseMenu.MainMenu) {
            ...
            menu_typefullscreen = this.m_GlobalConfig.menu_main_typefullscreen;
        }
            ...
            menu_typefullscreen = this.m_GlobalConfig.menu_result_typefullscreen;
        ...
        //显示分类全屏
        if (menu_typefullscreen) {
            //显示全屏
            this.ShowTgTypeFullScreen(null, _parent);
        }
    }
     ...
     * @param {Function} _closeCallFun 界面关闭回调函数
     */
    public ShowTgTypeFullScreen(_closeCallFun: Function = null, _parent: cc.Node = null) {
        // if (!BasePlatform.getInstance().IsWeChat()) {
        //     if (_closeCallFun) {
        //         _closeCallFun();
        //     }
        ...
        this.m_TgTypeFullBanner = true;
        let extData = {
            parent: _parent,
            startJump: this.m_GlobalConfig.typefullscreen_startjump,
            randjumpzjkj: this.m_GlobalConfig.randjump_zjkj,
            jumpCallFun: (_tgdata, _tgpos) => {
                this.JumpToProgram(_tgdata, _tgpos, _tgpos, () => {
                    //跳转成功
        ...
        TgAdvSdk.getInstance().ShowTgTypeFullScreen(extData);
    }
    ...
    */
    ...
    private InitWeChatBanner() {
        if (BasePlatform.getInstance().IsWeChat()) {
            ...
            if (!this.m_WeChatBannerID) {
                this.m_WeChatBannerIndex = Utils.RandNum(0, WeChatAdvConfig.BannerAdUnitIdList.length - 1);
                console.log("InitWeChatBanner this.m_WeChatBannerIndex", this.m_WeChatBannerIndex);
                this.m_WeChatBannerID = WeChatAdvConfig.BannerAdUnitIdList[this.m_WeChatBannerIndex];
            }
            ...
            if (this.m_WeChatBanner) {
                this.m_WeChatBanner.destroy();
            }
            this.AdvLog('banner this.m_WeChatBannerID', this.m_WeChatBannerID);
            this.m_WeChatBanner = WeChat.CreateBannerAd(this.m_WeChatBannerID, left, top, width, height);
        ...
        if (BasePlatform.getInstance().IsWeChat()) {
            ...
            if (BasePlatform.getInstance().IsWeChat()) {
                if (this.m_WeChatBanner) {
                    ...
                    this.m_WeChatBannerID = WeChatAdvConfig.BannerAdUnitIdList[this.m_WeChatBannerIndex];
                    //每次显示banner更换广告id
                    if (this.m_WeChatBannerIndex < WeChatAdvConfig.BannerAdUnitIdList.length - 1) {
                        this.m_WeChatBannerIndex++;
                    }
                    else {
                        this.m_WeChatBannerIndex = 0;
                    }
                    // 初始化banner
                    this.InitWeChatBanner();
                }
